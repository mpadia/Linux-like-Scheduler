...
int	rdyheadreal, rdytailreal;	/* head/tail of real process ready list (q indicies)	*/
...
//	int userpid;
	...
	// Changes make to ensure that at time of initizalization scheduler policy is default
...
LOCAL int sysinit()
{
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	rdytailreal = 1 + (rdyheadreal=newqueue());/* initialize real ready list */
