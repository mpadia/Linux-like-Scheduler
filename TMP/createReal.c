					...
					/* array in the code)		*/
{
	...
	if (((saddr = (unsigned long *)getstk(ssize)) ==
	    (unsigned long *)SYSERR ) || schedPolicy == LINUXSCHED || schedPolicy == DEFAULTSCHED ||
	...
	// Chagnes for new scheduler policy
	pptr->counter = REALQUANTUM;
        pptr->ocounter = REALQUANTUM;
        pptr->nprio = priority;
        pptr->chgprio = 0;
        pptr->ncreate = 1;
	
	// Changes to handle Real-Time processes
	pptr->realproc = 1;
	// ^^ changes end
